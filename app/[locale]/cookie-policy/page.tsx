...
function getCookiePolicy(locale: string) {
  try {
    const filePath = path.join(process.cwd(), 'content', 'legal', 'cookie', `${locale}.json`);
    const fileContent = fs.readFileSync(filePath, 'utf-8');
    return JSON.parse(fileContent);
  } catch (error) {
    console.error(`Failed to read cookie policy for ${locale}`, error);
    return { title: 'Cookie Policy', subtitle: '', last_updated: '', sections: [] };
  }
}
...
export async function generateMetadata({
  ...
  const content = getCookiePolicy(locale);
